...
QString MVPublic::getSerialNumber()
{
...
QString MVPublic::getMVSerialNumber()
{
    QDateTime time = QDateTime::currentDateTime();//获取系统现在的时间
    QString number = time.toString("yyyyMMddhhmmss"); //设置显示格式
    QTime t;
    t= QTime::currentTime();
    qsrand(t.msec()+t.second()*1000);
    int n = qrand();
    return "MV"+number+QString("%1").arg(n);//添加一个随机数
}
